...
def prepare_input(slice, tokenizer, max_len_pad_set, mlb):
    ...
    for group_name, df_group in grouped_slice:
        ...
        if len(mask_list_entry[-1]) < max_len_pad:
            padding = [
                [0] * MAX_LEN_SEQ for x in range(max_len_pad - len(mask_list_entry[-1]))
            ]
    ...
    inputs_tf = [context_ids, attention_mask, context_masks_tf]

    return inputs_tf, labels_tf, grouped_slice


def prepare_input_pt(slice, tokenizer, max_len_pad_set, mlb, device):
    list_contexts = []
    context_masks = []
    labels = []
    ...
    context_masks_pt = torch.from_numpy(np.array(context_masks)).to(device)
    ...
    labels_pt = torch.from_numpy(np.array(labels_mlb)).to(device)
    ...
    inputs_tf = [context_ids, attention_mask, context_masks_pt]
    print("Labesl shape: {}\n".format(context_masks_pt.shape))

    return inputs_tf, labels_pt, grouped_slice
