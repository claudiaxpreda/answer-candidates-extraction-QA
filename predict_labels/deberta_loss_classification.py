...
def create_model(encoder_model_name):
    if encoder_model_name == "google-bert/bert-base-uncased":
        encoder = TFBertModel.from_pretrained(
            encoder_model_name, output_hidden_states=True
        )
    else:
        encoder = TFDebertaV2Model.from_pretrained(
            encoder_model_name, output_hidden_states=True
        )
    ...
    embedding = encoder.deberta(context_ids, attention_mask=attention_mask)[0]
    ...
    product_step_b = tf.math.count_nonzero(
        questions_ids, 2, keepdims=True, dtype=tf.dtypes.float32
    )
    ...
    layer3 = layers.Dense(LABELS_NUM, name="DenseLayer3", activation="softmax")(layer2)
...
def main(answer_selection_model, max_len_pad, epochs, encoder_model_name):
    ...
    x_train, y_train, _ = pic.prepare_input(
        input_train_df, tokenizer, int(max_len_pad), mlb
    )
    x_val, y_val, _ = pic.prepare_input(
        input_validation_df, tokenizer, int(max_len_pad), mlb
    )
